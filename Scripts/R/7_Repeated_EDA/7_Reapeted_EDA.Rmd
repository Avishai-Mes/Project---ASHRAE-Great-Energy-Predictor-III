...
##                ---  Repeated Exploratory Data Analysis (EDA)  ---
...
Columns that have been categorized in "Data Cleansing" notebook need to be 
factorized again following reimport of the dataset. In previous notebook, a total 
of 27 nominal variables were factorized, along with an additional 6 variables 
that were categorized during outlier handling and had to be factorized at the end 
of the process.At this point I will factorize only those 6 categorized variables, 
and continue factorizing the rest 27 nominals in a later stage (after the use of 
'corr.test' function) in this notebook, since some of the nominals need to be 
used in the 'corr.test' which only accepts numerics.

```{r}
to_be_factorized_categorization_cols <- c('precip_depth_sum', 'precip_depth_min', 'precip_depth_max', 'precip_depth_sd', 'rel_humid_max', 'elevation_mode', 'year_built')
```
...
2. 'precip_depth_sum',  'precip_depth_min', 'precip_depth_max', 'precip_depth_sd' 
   - up to Q3 values are all zeros, therefore the columns considered sparse. 4
   Binary columns were created in 'Data Cleansing' notebook distinguish between 
   the zeros and non-zeros values of those columns.
...
```{r}
# export the table to excel, that would be easier to read the 490 rows
vars_for_Table1 <- names(ff_train_agg[,!names(ff_train_agg) %in% c("building_id", "date")])
Table1(data = ff_train_agg, miss = 3, x = vars_for_Table1, excel = 1, excel_file = "C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/Variable distribution-after data cleansing/Descriptive Statistics-Table1.xlsx")
```
...
suppressWarnings(exploreData(data = sample_df, y = "meter_reading_sum", 
                             dir = "C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/variable distribution-after data cleansing"))
```
...
Scatter plots between all pairs of variables, in order to discover patterns:
...
start_time <- system.time({
  # For runtime considerations, a random sample of data is taken:
  set.seed(2)
  sample_df <- ff_train_agg %>% slice_sample(n = 4000)
  ...
  pdf("C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/Graphs-after completion of data cleansing/scatter_plots_combinations_X_vars.pdf")
...
continuous and ordinal (including binary) variables in the presence of 
...
```{r}
pdf("C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/Graphs-after completion of data cleansing/Correlation Graph.pdf")
...
According to 'corr.test', 51 pairs of variables have correlation coefficient 
...
cat('\n', "Number of variable pairs with significant moderate correlation (0.2 - 0.7):", pair_count/2)  # print the total count
```

According to 'corr.test', 250 pairs of variables have correlation coefficient 
...
in the data. Variables in a range of values lower than 0.3 are considered as
independent.
...
The nominal variables will be factorized prior to the use in the mathematical 
function - 'cv.test'(Cramer's V) for calculating correlation coefficients.
As a good practice default, I prefer to factorize all nominal variables right 
after import of data. I didn't do it in this notebook, as variables start with 
"sw" and "is" which are nominal, were part of the input to 'corr.test' function 
used before, and this function gets only numeric values (factor values will cause 
a problem). Those binary variables could be considered also as ordinal for the 
function use, as they have only one distance between their values.
...
```{r}
# including the variables that have factorized in 'Data Cleansing' notebook
nominal <- c('site_id', 'building_id',  
                       ...
                       to_be_factorized_categorization_cols)
...
**'Cramer's V' (CV)** - a 'Chi-Squared'-based calculation of correlation 
...
'date' with 2 date dependent variables - 'is_weekend_mode' with full correlation, 
and 'is_holiday_mode' with a very high correlation and the only one that is not 
...
make sure that excel file 'cat_var_after_data_cleansing' is updated.

```{r}
pdf("C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/Graphs-after completion of data cleansing/meter_reading_sum_log_uni_graphs.pdf")
...
cat_lst <- read.csv("C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/csv files/cat_var_after_data_cleansing.csv")
...
'site_id'
'sw_primary_use_Parking_mode'
'sw_primary_use_ReligiousWorship_mode'
'sw_primary_use_Utility_mode'.
...
'boxplot' - describes the behavior of numeric variable's distribution
...
```{r}
pdf("C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/Graphs-after completion of data cleansing/boxplot_uni_graphs.pdf")
options(repr.plot.width = 8, repr.plot.height = 8)      # graph size
...
'scatter plot' - shows outliers of attributes. Horizontal axis is an index.
...
pdf("C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/Graphs-after completion of data cleansing/scatter_uni_graphs.pdf")
...
A heatmap of the missing data - only on columns that have missing values:

'vis_miss' function:

```{r}
pdf("C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/Graphs-after completion of data cleansing/Heatmap Missing Percentage Graph.pdf")
...
categories. Therefore I will add to the column 0 and 1 as levels, just for that 
reason.

```{r}
pdf("C:/Users/Avishai/Documents/General/Personal/DS Course/Google drive content/Projects/Final project-ASHRAE-Great Energy Predictor III/Graphs-after completion of data cleansing/Missingness plot.pdf")
...
```{r}
unique_lengths <- sapply(ff_train_agg, function(col) length(unique(col)))
unique_lengths
```
